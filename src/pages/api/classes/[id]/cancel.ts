...
import { cancelClass } from "@/lib/classes.mutation.handler";
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await cancelClass(supabase, context.params.id ?? "");
  return result.ok ? resultRedirect("CANCELLED") : resultRedirect(result.code);
};
